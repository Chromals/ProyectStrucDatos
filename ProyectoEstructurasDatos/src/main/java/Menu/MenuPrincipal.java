...
public class MenuPrincipal {
    ...
    static List<NodosP> listaProds= new ArrayList<>();
    ...
    private static void SubMenuProductos() {
        ...
        switch (seleccionProd) {
            ...
            case 2: {
                ...
                {
                    listaProds.add(productoSeleccionado);
                    carrito.setListaProductos(listaProds);
                }
            }
            default: {
                menuPrincipal();
    ...
    private static void SubMenuCarrito() {
        ...
        switch (seleccionProd) {
            ...
            case 2: //Finalizar Compra
            {
                ...
                JTextField encabezadoField = new JTextField(10);
                JTextField fechaField = new JTextField(10);
                JTextField medioPagoField = new JTextField(10);
                JPanel myPanel = new JPanel();
                myPanel.add(new JLabel("Encabezado de la Factura:"));
                myPanel.add(encabezadoField);
                myPanel.add(Box.createHorizontalStrut(15)); // Una separación horizontal
                myPanel.add(new JLabel("Fecha:"));
                myPanel.add(fechaField);
                myPanel.add(Box.createHorizontalStrut(15)); // Una separación horizontal
                myPanel.add(new JLabel("Medio de Pago:"));
                myPanel.add(medioPagoField);
                ...
                if (result == JOptionPane.OK_OPTION) {
                    ...
                    if (encabezadoField.getText().isEmpty() || fechaField.getText().isEmpty() || medioPagoField.getText().isEmpty()) {
                        JOptionPane.showMessageDialog(null, "Envio campos vacios, porfavor intente agregar una Factura Valido");
                        SubMenuCarrito();
                        return;
                    } else {
                        String enc = encabezadoField.getText();
                        ...
                        double monto = 0;
                        for(NodosP producto : c.getListaProductos()) {
                            monto += producto.getPrecio();
                        }
                        //factura.setMonto(monto);
                        
                        Facturacion f = new Facturacion(enc, fec, c.getListaProductos(), monto, mediopago, clienteSeleccionado);
                        fact.agregarFactura(f);
                    }
    ...
    private static void SubMenuFacturacion() {
        ...
        switch (seleccionProd) {
            case 0: {
                fact.consultarFacturas();
                SubMenuFacturacion();
                break;
            }
            case 1: {
                
                String respuesta = JOptionPane.showInputDialog(null, "Digite el Codigo de la Factura a consultar: ", "Busqueda De Factura ", JOptionPane.PLAIN_MESSAGE);
                
                fact.consultarFactura(Integer.parseInt(respuesta));
                SubMenuFacturacion();
                break;
            }
            case 2: {
                String respuesta = JOptionPane.showInputDialog(null, "Digite el Codigo de la Factura a Pagar: ", "Busqueda De Factura ", JOptionPane.PLAIN_MESSAGE);
                
                fact.consultarFactura(Integer.parseInt(respuesta));
                
                if(fact.elimina(Integer.parseInt(respuesta)))
                {
                    JOptionPane.showMessageDialog(null, "Factura Pagada Correctamente, se elimino de la lista");
                }
                else
                {
                    JOptionPane.showMessageDialog(null, "Factura no fue pagada, ocurrio un error");
                }
